...
def get_args():
    ...
    parser.add_argument('--TSModel', help='Model to use for time series part of data, the options are: 1. LSTM: LSTM model 2. BiLSTM: Bidirectional LSTM model', default='BiLSTM')
...
def get_embedding_dict(conf):
    ...
    index_to_key_tensor = data["model"]["index_to_key"]    #index2word-> index_to_key
    index_to_key_tensor.pop()  
    index_to_key_tensor.append('<pad>')
    word2index_lookup = {word: index for index,
                         word in enumerate(index_to_key_tensor)}
...
def get_text_sep(textdict, raw, timedict, start_times):
    ...
    suceed = 0
    missing = 0
    print("text",text,"X",X_data,"Y",Ys,"name",names,"time",times,"start time",start_times_arr)
