...
sys.path.append('/home/zmx/mimic3-benchmarks-master/')
...
import numpy as np
from mimic3models.impute import CrossNet
...
# Loading BioWordVec word embeddings
    ...
    print("word2index_lookup",word2index_lookup,"vectors",vectors)
    ...
    with torch.autograd.set_detect_anomaly(True):  # Error catcher
        for step, sample in enumerate(batch):
           # print(sample[0],sample[1],sample[2],sample[3])
            ...
            if args['notes_aggeregate'] == 'Mean' or args['notes_aggeregate'] == 'TimeAttn' or args['notes_aggeregate'] == 'Attn':
                text = [torch.tensor(x, dtype=torch.long).to(device) for x in sample[2]]
                text=torch.stack(text)   ## change    list-> tensor
              #  print("text 1",text.shape,type(text))   # (5,128)
                attn = [torch.tensor(x, dtype=torch.long).to(device) for x in sample[3]]
                attn=torch.stack(attn)
               # print("atten",attn,type(attn))
           ...
           # print("X",X,"text",text,"atten",attn,"times",times)
           
           # print("X",step,X.shape)  #(5,48,49)
            Logits, Probs = model(X, text, attn, times)
            Lambd = torch.tensor(0.01).to(device)
            l2_reg = model.get_l2()
...
def concat_text_timeseries(data_reader, data_raw):
   ...
   # print("how",train_text, data_raw,train_times, start_time)
    if args['notes_aggeregate'] == 'Flat':
...
def Read_Aggregate_data(mode, AggeragetNotesStrategies, discretizer=None, normalizer = None):
  ...
  #  dataPath = os.path.join('Data', mode + '_data_' +  File_AggeragetNotesStrategies + '.pkl')
    ...
    dataPath = os.path.join('/home/zmx/ClinicalNotesICU-master/data-all/root/', mode + '_data_' +  File_AggeragetNotesStrategies + '.pkl')
    if os.path.isfile(dataPath):
        ...
        print("mode!",mode)
       # if normalizer is not None:   #None -->not None
        discretizer = Discretizer(timestep=float(conf.timestep),
                              store_masks=True,
                              impute_strategy='previous',
                              start_time='zero')
       # print("size ??",reader.read_example(0)["X"].shape)  #(xx,69)
        discretizer_header = discretizer.transform(
            reader.read_example(0)["X"],header=reader.read_example(0)["header"])[1].split(',')
       ...
       # normalizer = None
        ...
        train_raw = ihm_utils.load_data1(   #train_raw->train_origin
            reader, discretizer, normalizer, conf.small_part, return_names=True)  #(5,48,76)  or (5,48,59)
        #print("train_raw",np.array(train_raw['data']).shape)
        
        print("Number of train_raw_names: ", len(train_raw['names'])) #14681 
               
        train_orin=train_raw
        #print("train_orin",train_orin[0],np.array(train_orin[0]).shape,train_orin[1])  #train_orin[0]).shape  (41,48,59)
        n=len(train_orin['data'][0])
        
       # print("n",n)
        #ts_data=np.array(None, ndmin=3)
        ts_data= np.array(train_orin['data'][0])
        ts_masks = train_orin['mask'] #np.random.randint(2, size=ts_data.shape)
        
        static_data = np.random.rand(n, static_size)   #(41,20)
        static_masks = np.random.randint(2, size=static_data.shape)
        
        labels = train_orin['data'][1]

        imputemodel = CrossNet(static_size, ts_size, rnn_hid_size, ts_len, impute_weight)
        if torch.cuda.is_available():
          imputemodel = imputemodel.cuda()
        
        optimizer = optim.Adam(imputemodel.parameters(), lr=1e-3)
        
        total_params = sum(p.numel() for p in imputemodel.parameters() if p.requires_grad)
        print('Total params is {}'.format(total_params))
        
        ts_data, ts_masks, static_data, static_masks, labels = torch.FloatTensor(ts_data).cuda(), \
        torch.FloatTensor(ts_masks).cuda(), torch.FloatTensor(static_data).cuda(), \
        torch.FloatTensor(static_masks).cuda(), torch.FloatTensor(labels).cuda()
        
        for epoch in range(epochs):
          imputemodel.train()
          ret = imputemodel.run_on_batch(ts_data, ts_masks, static_data, static_masks, labels, optimizer, epoch)
          
        print("imputation!",ret['imputations'].shape)   #torch (14681,48,59)
        print('loss:', ret['loss'])
        
        impuations=ret['imputations'].cpu().detach().numpy() 
       # print("impuations",type(impuations))
        train_raw1=ihm_utils.imput_name(impuations,reader, discretizer, normalizer, conf.small_part, return_names=True) 
        
       # print("train_raw1",train_raw1,type(train_raw1))
      #  train_raw=train_raw1 
       # print("train_raw",train_raw,type(train_raw))
    ...
    data_y = data[1]
    data_text = data[2]
   # print("texts",data_text,np.array(data_text).shape)#(7,)  11218
    data_names = data[3]
    data_times = data[4]
    start_times = data[5]
    #print("data_X",data_X,"data_y",data_y,"data_text",data_text,"data_names",data_names,"data_times",data_times)
    #print("data_times",data_times,"start_times",start_times)
    timetoends = get_time_to_end_diffs(data_times, start_times)
    #print("timetoends",timetoends)
    if args['model_name'] == 'BioBert':
        tokenized_path = os.path.join('/home/zmx/ClinicalNotesICU-master/data-all/root/', EmbedModelName + '_tokenized_ids_attns_' + mode + '_' + File_AggeragetNotesStrategies + '_' + str(args['MaxLen']) + '_' + args['NoteSplit'] + '.pkl')
        if os.path.isfile(tokenized_path):
         ...
         #print("txt_ids",txt_ids)
          with open(tokenized_path, 'wb') as f:   
              pickle.dump((txt_ids, attention_masks), f)# Write the output of tokenizer to a pickle file so we can use it later
       ...
       # print("mode",mode,"attention_masks",attention_masks)
    ...
    del data
    #print("mode",mode,"data_X",data_X,"data_y",data_y)
...
def generate_padded_batches(x, y, t, text_ids, data_attn, data_times, batch_size, w2i_lookup):
    ...
    batches = []
    begin = 0
    #print("ids",text_ids,np.array(text_ids).shape)  #11181
   # print("text",np.array(t).shape)
...
if bool(int(args['load_model'])):
    ...
    last_best_val_aucpr, _, probs, val_auc = validate(model, data_X, data_y, data_text, txt_ids, attention_masks, data_times, 
                                                      data_names, batch_size, word2index_lookup, last_best_val_aucpr, False)
    print('Model loaded')
...
if args['mode'] == 'eval':
    ...
    write_probs(eval_data_names, eval_data_text, probs, eval_data_y,  ModelNameOut + "_val.csv")
...
if args['mode'] == 'test':
    ...
    write_probs(test_data_names, test_data_text, probs, test_data_y,  ModelNameOut + ".csv")
