...
class getTextEmb():     
  def __init__(self, dropout_keep_prob, W_embed, model_name, model_type, BioBert, notes_aggeregate, TSModel, TS_aggeregate, Attn_Type, device):
        ...
        if model_name == 'avg_we':
            ...
            print("avg text_embed_size",text_embed_size)
        elif model_name == 'cnn':
            print('Using CNN for text part')
            text_embed_size =  len(self.sizes) * 256    # len=3
            print("cnn text_embed_size",text_embed_size)   #768
        elif model_name == 'BioBert':
            print('Using BioBert model')
            text_embed_size =  BioBert.config.hidden_size
            print("text_embed_size",text_embed_size)
    ...
    def __init__(self, dropout_keep_prob, W_embed, model_name, model_type, BioBert, notes_aggeregate, TSModel, TS_aggeregate, Attn_Type, device):
        ...
        if TSModel == 'BiLSTM':
          #  self.lstm = nn.LSTM(76, time_series_size, 1, batch_first=True, bidirectional=True)        
            self.lstm = nn.LSTM(59, time_series_size, 1, batch_first=True, bidirectional=True)  
        else:
           # self.lstm = nn.LSTM(76, time_series_size, 1, batch_first=True)
            self.lstm = nn.LSTM(59, time_series_size, 1, batch_first=True)
      ...
      #  print("X",X.shape)
        ...
        if self.model_type != 'baseline':
            if self.model_name != 'BioBert':
               ...
               # print("text",text,type(text),text.shape)   #(5,128)
                embeds = self.W_embed(text)
               # print("embed",embeds,embeds.shape)  #(5,128,200)  5 notes  128  200dim
            if self.model_name == 'avg_we':
                text_embeddings = torch.mean(embeds, axis=1)
                print("avg embed",text_embeddings ,text_embeddings.shape)  #(5,200)
                ...
                if self.notes_aggeregate == 'Mean' or  self.notes_aggeregate == 'TimeAttn' or self.notes_aggeregate == 'Attn':
                    ...
                    for txts, attn, ts in zip(text, attns, times):
                        ...
                        #print("Bio txtemb",txtemb,txtemb.shape)
                        emb = txtemb[0][:,0,:]
                        print("Bio emb",emb,emb.shape)
                        if self.notes_aggeregate == 'TimeAttn':
                    ...
                    print("Bio text_embeddings",text_embeddings,text_embeddings.shape) #(5,768)
               ...
               # print("cnn  text_embeddings!",text_embeddings,text_embeddings.shape)   #(5,768)
        # Getting the time-series part embedding
        rnn_outputs, _ = self.lstm(X)    
      #  print("rnn_outputs",rnn_outputs,rnn_outputs.shape)  #(5,48,512)
        ...
        #    print("x? text_embeddings",text_embeddings.shape,mean_rnn_outputs,mean_rnn_outputs.shape)#(5,578)  (5,512)
